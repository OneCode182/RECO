...
function Show-NetworkMenu {
    Clear-Host
    Write-Host "======= NETWORK INFORMATION MENU ======="
...
function Run-NetworkCommand {
    param ([int]$option)
    switch ($option) {
        1 { Get-NetIPConfiguration | Format-Table }
        2 { Get-NetRoute | Format-Table }
        3 { Get-NetTCPConnection | Format-Table }
        4 { Get-NetAdapterStatistics | Format-Table }
        5 { ipconfig /all }
        6 { Exit }
...
if ($connection) {
    Write-Host "YES: Port $Port is open."
    ...
    Write-Host "NO: Port $Port is not open."
}




Guarda este archivo como CheckPort.ps1 y ejecútalo desde PowerShell así:

.\CheckPort.ps1 -Port 80



Chat me da este también, es una combinación de los dos 
